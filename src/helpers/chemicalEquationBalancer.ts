...
// Balances the given formula string and returns the HTML output.
function balance(formulaStr: string): string {
    // Clear output
    ...
    try {
        // Set up matrix
        let matrix: Matrix = buildMatrix(eqn);
        // Solve linear system
        solve(matrix);
        // Get coefficients
        const coefs: Array<number> = extractCoefficients(matrix);
        // Self-test, should not fail
...
class Parser {
    private tok: Tokenizer; // eslint-disable-line
        ...
        while (true) {
            ...
            if (next === null) break;
                ...
                throw {
                    ...
                    message: 'Tikimasi + arba pabaigos',
                    start: this.tok.pos,
                };
        }
    ...
    private parseTerm(): Term {
        ...
        while (true) {
            ...
            if (next == '(') items.push(this.parseGroup());
                ...
                throw {
                    // message: 'Invalid term - number not expected',
                    message: 'Ištrink pabrauktą skaičių',
                    start: this.tok.pos,
                };
            else break;
        }
        ...
        if (next == '^') {
            ...
            if (next === null)
                throw {
                    ...
                    message: 'Tikimasi skaičiaus arba ženklo',
                    start: this.tok.pos,
                };
            else {
        ...
        if (electron) {
            if (items.length > 0)
                throw {
                    ...
                    message: 'Sintaksės klaida - elektronai turi eiti atskirai',
                    start: startPos,
                    end: this.tok.pos,
                };
            if (charge === null)
            ...
            if (charge != -1)
                throw {
                    ...
                    message: 'Sintaksės klaida - netinkamas elektrono krūvis',
                    start: startPos,
                    end: this.tok.pos,
                };
        } else {
            if (items.length == 0)
                throw {
                    ...
                    message: 'Sintaksės klaida - trūksta info',
                    start: startPos,
                    end: this.tok.pos,
                };
        ...
        while (true) {
            ...
            if (next == '(') items.push(this.parseGroup());
                ...
                throw {
                    ...
                    message:
                        'Tikimasi elemento, grupės ar uždaromajo skliaustelio',
                    start: this.tok.pos,
                };
        }
    ...
    private parseElement(): ChemElem {
        ...
        if (!/^[A-Z][a-z]*$/.test(name)) throw 'Assertion error';
...
class Tokenizer {
    ...
    public peek(): string | null {
        if (this.pos == this.str.length)
            // End of stream
            return null;
        const match: RegExpExecArray | null = /^([A-Za-z][a-z]*|[0-9]+|[+\-^=()])/.exec(
    ...
    private skipSpaces(): void {
        ...
        if (match === null) throw 'Assertion error';
...
class Term {
    ...
    public toHtml(): HTMLElement {
        ...
        if (this.items.length == 0 && this.charge == -1) {
            ...
            for (const item of this.items) node.appendChild(item.toHtml());
            if (this.charge != 0) {
                let s;
...
class ChemElem {
    public constructor(private name: string, private count: number) {
        if (count < 1)
            throw 'Assertion error: Count must be a positive integer';
    }
...
class Matrix {
    ...
    public constructor(public numRows: number, public numCols: number) {
        if (numRows < 0 || numCols < 0) throw 'Illegal argument';
    ...
    public gaussJordanEliminate(): void {
        // Simplify all rows
        let cells: Array<Array<number>> = (this.cells = this.cells.map(
            Matrix.simplifyRow
        ));
...
function solve(matrix: Matrix): void {
    ...
    if (i == matrix.numRows - 1)
        ...
        throw 'Visi koeficientai yra nuliai'; // Unique solution with all coefficients zero
...
function extractCoefficients(matrix: Matrix): Array<number> {
    ...
    if (cols - 1 > rows || matrix.get(cols - 2, cols - 2) == 0)
        ...
        throw 'Galimi keli nepriklausomi sprendimai (ko gero perdaug elementų)';
    ...
    if (allzero)
        ...
        throw 'Visi koeficientai yra nuliai';
    return coefs;
}
...
function checkOverflow(x: number): number {
    if (Math.abs(x) >= INT_MAX) throw 'Arithmetic overflow';
    return x;
}
